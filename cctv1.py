                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        if names[c] == 'person' or names[c] == 'motorcycle' or names[c] == 'bycbicycle' :
                            ...
                            gambar = f'{id}.jpg'
                            count_person(bboxes,w,h,id,dir2,im0, gambar, dir3)
...
def count_car(box,w,h,id):
    ...
    if int(box[1]+(box[3]-box[1])/2) > (550) and int(box[1]+(box[3]-box[1])/2) < (640):
        if  id not in data1:
            car += 1
            data1.append(id)
def count_person(box,w,h,id, dir2,im0, gambar, dir3):
    ...
    if int(box[1]+(box[3]-box[1])/2) > (0):
        if  id not in data2:
            ...
            # Log in and send the email   
            s.sendmail(email_sender, email_receiver, em.as_string())
            inputpelanggaran(jp, gambar, dir3)
            person = 0
...
def inputdata(car, truck, count):
    ...
    sql = "INSERT INTO grb_kalukubadoa(waktu_input, Mobil, Bus_Truk, total) VALUES (now(), '"+car+"', '"+truck+"', '"+count+"')";
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='satu', help='save results to project/name')
