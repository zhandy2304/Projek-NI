...
def current(opt):
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if show_vid:
                global masuk, keluar, count1, car1, truck1, bus
                color = (200, 255, 100)
                color1 = (0, 255, 255)
                start_point = (0, 190)
                end_point = (220, 220)
                ...
                cv2.putText(im0, 'OUT = ' + str(keluar), org1, font,
                            fontScale, color, thickness, cv2.LINE_AA)
...
def detect_current(box, w, h, id, im0, simpan, dir3):
    ...
    if int(box[1]+(box[3]-box[1])/2) > (190) and int(box[1]+(box[3]-box[1])/2) < (220) and int(box[0]+(box[2]-box[0])/2) < (140) :
        if id not in data1:
            masuk += 1
            data1.append(id)
    ...
    if int(box[1]+(box[3]-box[1])/2) > (220) and int(box[1]+(box[3]-box[1])/2) < (300) and int(box[0]+(box[2]-box[0])/2) < (140) :
        if id not in data1:
            keluar += 1
            data1.append(id)
...
def inputdata(jumlah):
    ...
    sql = "INSERT INTO off_ramp_rappokalling(waktu_input, jumlah) VALUES (now(), '"+jumlah+"')"
