    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if show_vid:
                ...
                color = (0, 255, 0)
                start_point = (1220, 920)
                end_point = (1560, 930)
                start_point1 = (730, 1200)
                end_point1 = (1550, 1250)
...
def count_obj1(box, w, h, id):
    ...
    if int(box[1]+(box[3]-box[1])/2) > (910) and int(box[1]+(box[3]-box[1])/2) < (1300):
        if id not in data:
            count1 = truck1 + car1
            data.append(id)
...
def count_car(box, w, h, id):
    ...
    center_coordinates = (
        int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])/2))
    if int(box[1]+(box[3]-box[1])/2) > (910) and int(box[1]+(box[3]-box[1])/2) < (1300) and int(box[0]+(box[2]-box[0])/2) < (1560) and int(box[0]+(box[2]-box[0])/2) > (1210):
        if id not in data1:
            car1 += 1
            data1.append(id)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source3', type=str,
                        default='rtsp://admin:admin123@192.168.3.19:554/live1s1.sdp', help='source')
