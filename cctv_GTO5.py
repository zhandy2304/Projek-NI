...
def gto(opt):
    ...
    out, source4, yolo_model, deep_sort_model, show_vid, save_vid, save_txt, imgsz, evaluate, half, project, name, exist_ok = \
        opt.output, opt.source4, opt.yolo_model, opt.deep_sort_model, opt.show_vid, opt.save_vid, \
        opt.save_txt, opt.imgsz, opt.evaluate, opt.half, opt.project, opt.name, opt.exist_ok
    webcam = source4 == '0' or source4.startswith(
        'rtsp') or source4.startswith('http') or source4.endswith('.txt')
    ...
    if webcam:
        ...
        dataset = LoadStreams(source4, img_size=imgsz,
                              stride=stride, auto=pt and not jit)
        bs = len(dataset)  # batch_size
    else:
        dataset = LoadImages(source4, img_size=imgsz,
                             stride=stride, auto=pt and not jit)
        bs = 1  # batch_size
    ...
    txt_file_name = source4.split('/')[-1].split('.')[0]
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        if names[c] == 'car' or names[c] == 'truck' or names[c] == 'bus':
                            ...
                            count_obj1(bboxes, w, h, id, dir2, im0, simpan, gambar,save_txt,txt_path)
                            if int(output[1]+(output[3]-output[1])/2) > (190) and int(output[1]+(output[3]-output[1])/2) < (450) and int(output[0]+(output[2]-output[0])/2) < (750) and int(output[0]+(output[2]-output[0])/2) > (450):
                                annotator.box_label(
            ...
            if show_vid:
                ...
                start_point = (500, 190)
                end_point = (750, 190)
                start_point1 = (500, 450)
                end_point1 = (750, 450)
                start_point2 = (500, 190)
                end_point2 = (500, 450)
                start_point3 = (750, 190)
                end_point3 = (750, 450)
...
def video_feed():
    ...
    with torch.no_grad():
        return Response(gto(opt),
                        mimetype='multipart/x-mixed-replace; boundary=frame')


def count_obj1(box, w, h, id, dir2, im0, simpan, gambar, save_txt,txt_path):
    global count1, data, t6, t7
    center_coordinates = (
        int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])/2))
    if int(box[1]+(box[3]-box[1])/2) > (190) and int(box[1]+(box[3]-box[1])/2) < (450) and int(box[0]+(box[2]-box[0])/2) < (750) and int(box[0]+(box[2]-box[0])/2) > (450):
        if id not in data:
            count1 += 1
            data.append(id)
...
def inputcount(dir2, count1, simpan,gambar):
    with open(dir2, 'rb') as f:
        image_data = f.read()
        image_type = imghdr.what(f.name)
        image_name = f.name
    sql = "INSERT INTO gto5(num,tm, capture,img) VALUES ( '"+str(count1)+"', now(), '"+str(simpan)+ "','"+gambar+ "')"
    print(sql)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='Gto',
