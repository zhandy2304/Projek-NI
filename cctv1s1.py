...
def detect(opt):
    ...
    out, source, yolo_model, deep_sort_model, show_vid, save_crop, save_vid, save_txt, imgsz, evaluate, half, project, name, exist_ok= \
        opt.output, opt.source, opt.yolo_model, opt.deep_sort_model, opt.show_vid, opt.save_crop, opt.save_vid, \
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        if names[c] == 'person' or names[c] == 'motorcycle' or names[c] == 'bycbicycle' :
                            ...
                            count_person(bboxes,w,h,id, dir2)
...
def count_obj(box,w,h,id):
    global count,data, t7, t6, person, truck, car
    center_coordinates = (int(box[0]+(box[2]-box[0])/2) , int(box[1]+(box[3]-box[1])/2))
    if int(box[1]+(box[3]-box[1])/2) > (550) and int(box[1]+(box[3]-box[1])/2) < (700):
        if  id not in data:
            count = truck + car
            data.append(id)
...
def count_car(box,w,h,id):
    ...
    if int(box[1]+(box[3]-box[1])/2) > (550) and int(box[1]+(box[3]-box[1])/2) < (700):
        if  id not in data1:
            car += 1
            data1.append(id)
def count_person(box,w,h,id, dir2):
    global person,data2
    center_coordinates = (int(box[0]+(box[2]-box[0])/2) , int(box[1]+(box[3]-box[1])/2))
        ...
        if  id not in data2:
            ...
            #     s.sendmail(email_sender, email_receiver, em.as_string())
                for x in receiver_id:
                    ...
                    bot.sendMessage(x, 'Ada Pelanggaran Gerbang tol kaluku Badoa') 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='rtsp://admin:admin123@192.168.1.127:554/live.sdp', help='source')  # file/folder, 0 for webcam
