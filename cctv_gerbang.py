...
def gerbang(opt):
    ...
    out, source5, yolo_model, deep_sort_model, show_vid, save_vid, save_txt, imgsz, evaluate, half, project, name, exist_ok = \
        opt.output, opt.source5, opt.yolo_model, opt.deep_sort_model, opt.show_vid, opt.save_vid, \
        opt.save_txt, opt.imgsz, opt.evaluate, opt.half, opt.project, opt.name, opt.exist_ok
    webcam = source5 == '0' or source5.startswith(
        'rtsp') or source5.startswith('http') or source5.endswith('.txt')
    ...
    if webcam:
        ...
        dataset = LoadStreams(source5, img_size=imgsz,
                              stride=stride, auto=pt and not jit)
        bs = len(dataset)  # batch_size
    else:
        dataset = LoadImages(source5, img_size=imgsz,
                             stride=stride, auto=pt and not jit)
        bs = 1  # batch_size
    ...
    txt_file_name = source5.split('/')[-1].split('.')[0]
...
#     center_coordinates = (
#         int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])/2))
#     if int(box[1]+(box[3]-box[1])/2) > (h-280) and int(box[1]+(box[3]-box[1])/2) < (h-190) and int(box[0]+(box[2]-box[0])/2) < (410) and int(box[0]+(box[2]-box[0])/2) > (285):
#         if id not in data1:
#             car1 += 1
#             data1.append(id)
...
#     center_coordinates = (
#         int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])/2))
#     if int(box[1]+(box[3]-box[1])/2) > (h-340) and int(box[1]+(box[3]-box[1])/2) < (h-150) and int(box[0]+(box[2]-box[0])/2) < (410) and int(box[0]+(box[2]-box[0])/2) > (285):
#         if id not in data2:
#             person += 1
#             data2.append(id)
#             jp = 'Ada Orang'
